  ...
  if (!promptTemplate) {
    if (reportType === 'wish') {
      promptTemplate = currentLang === 'ja'
        ...
        2. 手法B：行動指引（Action-Oriented）- 具体的な「顕化ワーク」と転換のステップを提示する。実務的な悩みや行き詰まりを感じている時に最適。
        3. 手法C：視点の転換（Perspective Shift）- 悩みを「魂のギフト」として再定義する。すぐには変えられない状況や心の平安を求めている時に最適。
        
        【分析の指針】
        - ユーザーの悩み内容を分析し、最も効果的な手法（A, B, または C）を1つ選択してください。
        - 回答は以下の5つのセクションで構成し、最初の4つは「隠喩的、詩的、文学的」な表現を用い、最後は「平易な言葉による要約」としてください：
          1. 【魂の顕影】(soulMirror)：彼女の悩みに直接応え、カードのイメージを彼女の現状の深い反映へと昇華させます。
          2. 【運命の囁き】(destinyWhisper)：カードのキーワード、導きの言葉、そして彼女の現実の苦境を深く縫い合わせ、エネルギーの流れる方向を明らかにします。
          3. 【転換の導き】(transformationPath)：彼女が気づいていない盲点を見通し、悩みを再定義して、変化のきっかけを提供します。
        ...
        - カードの意味（70%）と悩みの文脈（30%）を融合させ、温かくも力強い言葉で綴ってください。
